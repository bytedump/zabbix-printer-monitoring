...
> A **Paper Alerts** table tops the view — empty while the fleet is healthy, and listing one row per affected printer with a colour-coded severity cell (red high, orange average, yellow warning) when a paper jam, out-of-paper, low-paper or open-door trigger fires. Below it: live toner levels for the whole fleet — colour printers as wide CMYK bars, B&W printers as big gauges, and offline units flagged in red, grouped into sections. The whole dashboard is generated by `create_dashboard.py`.
