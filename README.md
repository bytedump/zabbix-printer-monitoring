...
> A **paper-status headline** tops the view — one big tile that is green ("✅ ALL PAPER OK") while the fleet is healthy and flips red ("🚨 PAPER ALERT") the moment any printer reports a problem. Just below it a **detail banner** fills with severity-coloured rows (paper jam, out of paper, low paper, door open) per affected printer. Then live toner levels for the whole fleet — colour printers as CMYK bars, B&W printers as gauges, and offline units flagged in red, grouped into sections. The whole dashboard is generated by `create_dashboard.py`.
