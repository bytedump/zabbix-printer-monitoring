...
> A clean **Paper Alerts** table tops the view — empty while the fleet is healthy, and listing one row per affected printer (with a severity-coloured dot: 🔴 high, 🟠 average, 🟡 warning) when a paper jam, out-of-paper, low-paper or open-door trigger fires. Below it: live toner levels for the whole fleet — colour printers as CMYK bars, B&W printers as gauges, and offline units flagged in red, grouped into sections. The whole dashboard is generated by `create_dashboard.py`.
